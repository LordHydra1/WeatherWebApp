...
export class WeatherService {
  ...
  getWeatherDaily(city: string, country: string) {
    return this.http.get(`https://api.weatherbit.io/v2.0/current?city=${city}&country=${country}&lang=it&key=` + this.key);
  }
  ...
  getCountries() {
    return this.http.get(this.allCountries)
  }
  getCities(city:string){
    return this.http.get(`http://geodb-free-service.wirefreethought.com/v1/geo/cities?namePrefix=${city}&limit=5&offset=0&hateoasMode=false`)
  }
}
